...
export default function OfficialSchemesDirectory({ currentLang, onBack }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6 animate-in fade-in duration-200">
      <button onClick={onBack} className="flex items-center gap-1.5 text-gov-navy font-extrabold hover:underline">
        ...
        {isHindi ? 'होम पर वापस जाएँ' : 'Back to home'}
      </button>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-5">
          <div className="max-w-3xl">
            <div className="flex items-center gap-2 text-amber-300 font-extrabold text-sm uppercase tracking-wide">
              ...
              {isHindi ? 'आधिकारिक सरकारी निर्देशिका' : 'Official government directory'}
            </div>
            <h2 className="mt-2 text-3xl sm:text-4xl font-black">
              {isHindi ? 'भारत की सभी उपलब्ध योजनाएँ' : 'All available Government of India schemes'}
            </h2>
            <p className="mt-3 text-slate-200 text-base leading-relaxed">
              {isHindi
                ? 'केंद्र, राज्य और केंद्र शासित प्रदेशों की 4,770+ योजनाओं को भारत सरकार के myScheme पोर्टल पर खोजें। यह सूची आधिकारिक स्रोत से लगातार अपडेट होती रहती है।'
                : 'Search 4,770+ Central, State and Union Territory schemes through the Government of India’s myScheme portal. This catalogue is maintained at the official source.'}
            </p>
          </div>
          <a href={MY_SCHEME_URL} target="_blank" rel="noopener noreferrer" className="gov-btn-accent shrink-0 py-3 px-5 flex items-center justify-center gap-2 text-center">
            {isHindi ? 'सभी योजनाएँ खोजें' : 'Browse all schemes'} <ExternalLink className="w-5 h-5" />
          </a>
        </div>
      </section>
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 flex items-start gap-3 text-emerald-950">
        ...
        <p className="text-sm font-semibold leading-relaxed">
          {isHindi
            ...
            : 'SchemeSetu’s eligibility search only recommends schemes whose rules are verified here. For every available government scheme, open a category below and use the official myScheme catalogue for current details and applications.'}
        </p>
      </div>

      <section>
        <h3 className="text-2xl font-black text-gov-navy">{isHindi ? 'श्रेणी के अनुसार योजनाएँ' : 'Schemes by category'}</h3>
        <p className="mt-1 text-sm text-slate-600 font-medium">{isHindi ? 'myScheme पर प्रदर्शित वर्तमान संख्या' : 'Current counts shown by myScheme'}</p>
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {CATEGORIES.map(([name, nameHi, count]) => (
            <a key={name} href={MY_SCHEME_URL} target="_blank" rel="noopener noreferrer" className="gov-card p-4 hover:shadow-md hover:border-gov-saffron transition-all flex items-center justify-between gap-3">
              <div>
                <div className="font-extrabold text-gov-navy leading-snug">{isHindi ? nameHi : name}</div>
                <div className="mt-1 text-xs text-slate-500 font-bold">{count} {isHindi ? 'योजनाएँ' : 'schemes'}</div>
              </div>
      ...
      <p className="text-xs text-slate-500 text-center pb-2">
        {isHindi ? 'स्रोत: भारत सरकार का myScheme राष्ट्रीय प्लेटफ़ॉर्म। संख्या 5 सितंबर 2026 को सत्यापित की गई।' : 'Source: Government of India myScheme national platform. Counts verified on 5 September 2026.'}
      </p>
    </div>
  );
}
